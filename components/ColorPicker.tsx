...
export function ColorPicker({ onPick, onCancel }: { onPick: (c: CardColor) => void; onCancel: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md p-6">
      <div className="w-full max-w-xs rounded-2xl glass-panel border border-line p-5 animate-popIn">
        ...
        <div className="grid grid-cols-2 gap-3">
          {OPTIONS.map((o) => (
            <button
              ...
              className={`btn-tactile ${o.bg} h-20 rounded-xl font-display font-bold text-ink/90`}
            >
              {o.label}
            </button>
          ))}
