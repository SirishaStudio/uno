...
export function HostSettingsPanel({
  ...
  if (!isHost) {
    return (
      <div className="rounded-2xl border border-line bg-panel/60 p-4 text-sm text-white/60 space-y-1">
        <p>
        ...
        <p className="text-white/40 text-xs">Waiting for the host to start...</p>
      </div>
    );
  }

  return (
    <div className="rounded-2xl border border-line bg-panel/60 p-4 space-y-5">
      <div>
          ...
          {(["normal", "wild"] as const).map((m) => (
            <button
              ...
              className={`py-2 rounded-lg text-sm font-medium border transition-colors ${
                settings.gameMode === m
                  ? "border-uno-yellow text-uno-yellow bg-uno-yellow/10"
